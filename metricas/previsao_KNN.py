...
def obter_resultados_matriz_confusao_KNN(dados:Dados, divisao_treino, k_lista:List[int],\
     ...
     titulo:str="", save_fig:bool=False)-> Dict[str, Dict[str, Dict[str, str]]]:
    """
        ...
        Returns:
            Dict[str, Dict[str, Tuple[str, str, str]]]: Dicionario com os resultados de cada teste 
        ...
        for tamanho_treino in tamanho_treino_lista:
            ...
            for i in range(repeticoes_divisao):
                ...
                if i == 0: # imprime apenas uma vez
                    TE, PM, P = obter_matriz_confusao_KNN(dados, X_train, X_test, y_train, y_test, k,\
                     repeticoes_classificacao, titulo=str(Path(titulo) / f"k-{k}" / f"n-{tamanho_treino}"),\
                                            save_fig=save_fig)
                if i != 0: # imprime apenas uma vez
                    TE, PM, P = obter_matriz_confusao_KNN(dados, X_train, X_test, y_train, y_test, k,\
                         repeticoes_classificacao,titulo=str(Path(titulo) / f"k-{k}" / f"n-{tamanho_treino}"),\
